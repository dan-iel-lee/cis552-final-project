...
-- | Substitution Function: Takes a variable = expression, and substitutes expression for variable in the
-- | in the remaining portion of expression
...
-- | Checks if the expression is completely simplified
...
-- | Applies a function to the first x in a list which satisifies a predicate.
...
-- | Takes expression, and applies one simplification. Throws error on failure.
step :: Expression -> StepResult
step (App e es)
  | not (isValue e) = do
    s <- step e
    return $ App s es
...
-- | Completely simplifies expression to value if possible
eval :: Expression -> StepResult
eval e = do
  s <- step e
  if isValue s then return s else eval s

-- | Completely simplifies expression and prints steps along the way
...
-- | Parses a file, evaluates and returns result
topEval :: FilePath -> IO ()
topEval fs = do
  exp <- parseFile fs
  print exp
