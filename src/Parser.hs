...
varP = let potentialVar = wsP $ commentsP ((:) <$> P.satisfy Char.isLower <*> many (P.satisfy Char.isAlpha))
...
exprP = wsP $ commentsP appPP
  where
    ...
    baseP = {- inlineAnnotP <|> -}boolExprP <|> intExprP <|> ifP <|> lamP <|> letrecP <|> caseP
        <|> dcEP <|> varExprP
